        ...
        if SHAP_AVAILABLE and os.path.exists(model_file):
            try:
                with st.spinner("⏳ Đang tính toán SHAP values..."):
                    ...
                    # Lấy đúng feature names mà model đã được train
                    model_features = list(surrogate_model.feature_names_in_)
                    ...
                    # One-hot encode đúng như lúc train
                    df_surrogate = pd.get_dummies(df_surrogate, columns=['product_id', 'gender'])
                    df_surrogate = df_surrogate.fillna(0)
                    
                    cols_to_drop = [c for c in ['customer_id', 'total_qty'] if c in df_surrogate.columns]
                    df_surrogate = df_surrogate.drop(columns=cols_to_drop)
                    
                    # Căn chỉnh đúng theo feature_names_in_ của model
                    # Thêm cột thiếu = 0, bỏ cột thừa
                    X = df_surrogate.reindex(columns=model_features, fill_value=0)
                ...
                st.info("Kiểm tra: pip install shap>=0.50.0 scikit-learn>=1.4.0")
